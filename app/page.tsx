...
export default function Home() {
  ...
  /*
   * PHIM NỔI BẬT
   * Muốn đổi phim nổi bật thì chỉ cần đổi slug ở đây.
   */
  const featuredMovie =
    movies.find(
      (movie) => movie.slug === "nhan-ha-truong-ninh"
    ) ?? movies[0];
  ...
  useEffect(() => {
    ...
    async function loadHomeData() {
      ...
      if (error) {
        ...
        /*
         * Mỗi phim chỉ hiển thị 1 mục.
         * Lấy lần xem gần nhất của từng phim.
         */
  ...
  /*
   * GHÉP LỊCH SỬ XEM VỚI THÔNG TIN PHIM
   * Thông tin poster, tên phim, link...
   * đều lấy từ lib/movies.ts
   */
  const continueWatching = useMemo(() => {
    return watchHistory
      .map((item) => {
        const movie = movies.find(
          (m) => m.slug === item.movie_slug
        );
